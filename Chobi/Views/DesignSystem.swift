...
class ScaleNSView: NSView {
    ...
    override func hitTest(_ point: NSPoint) -> NSView? {
        ...
        let unscaledLocalPoint = super.convert(point, from: superview)
        if bounds.contains(unscaledLocalPoint) {
            return self
        }
        return nil
    }
    ...
    override func convert(_ point: NSPoint, from view: NSView?) -> NSPoint {
        let p = super.convert(point, from: view)
        return NSPoint(x: p.x / scale, y: p.y / scale)
    }

    override func convert(_ point: NSPoint, to view: NSView?) -> NSPoint {
        let p = NSPoint(x: point.x * scale, y: point.y * scale)
        return super.convert(p, to: view)
    }

    override func convert(_ size: NSSize, from view: NSView?) -> NSSize {
        let s = super.convert(size, from: view)
        return NSSize(width: s.width / scale, height: s.height / scale)
    }

    override func convert(_ size: NSSize, to view: NSView?) -> NSSize {
        let s = NSSize(width: size.width * scale, height: size.height * scale)
        return super.convert(s, to: view)
    }

    override func convert(_ rect: NSRect, from view: NSView?) -> NSRect {
        let r = super.convert(rect, from: view)
        return NSRect(
            x: r.origin.x / scale,
            y: r.origin.y / scale,
    ...
    override func convert(_ rect: NSRect, to view: NSView?) -> NSRect {
        ...
        return super.convert(r, to: view)
    }
}

struct ScaleWrapper<Content: View>: NSViewRepresentable {
    let scale: Double
    let content: Content

    init(scale: Double, @ViewBuilder content: () -> Content) {
        self.scale = scale
        self.content = content()
    }

    func makeNSView(context: Context) -> ScaleNSView {
        let hostingView = NSHostingView(rootView: content)
        return ScaleNSView(hostingView: hostingView)
    }

    func updateNSView(_ nsView: ScaleNSView, context: Context) {
        nsView.scale = CGFloat(scale)
...
public struct ZoomContainer<Content: View>: View {
    ...
    public var body: some View {
        ScaleWrapper(scale: max(0.5, min(2.0, zoomScale))) {
            content
        }
    }
}
