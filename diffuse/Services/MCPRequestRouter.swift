...
actor MCPRequestRouter {
    ...
    nonisolated static let tools: [MCPToolRegistration] = [
        MCPToolRegistration(
            name: "diffuse.list_workspaces",
            description: "List registered Diffuse workspaces and latest analysis status.",
            inputSchema: ["includeInactive": "boolean"]),
        MCPToolRegistration(
            name: "diffuse.get_current_review_context",
    ...
    nonisolated static let prompts: [MCPPromptRegistration] = [
        MCPPromptRegistration(
            name: "diffuse.review_change",
            description: "Use Diffuse context to review a change."),
        MCPPromptRegistration(
            name: "diffuse.understand_change",
            description: "Summarize a change using buckets, symbols, and profile evidence."),
        MCPPromptRegistration(
            name: "diffuse.write_review_comment",
            description: "Ground review comments in Diffuse findings, files, and symbols."),
    ]
    ...
    func callTool(name: String, arguments: [String: Any]) async -> MCPRouteResult {
        do {
            ...
            case "diffuse.search_review_context":
                return try success(
            ...
            case "diffuse.get_profile_context":
                return try success(
    ...
    func readResource(uri: String) async -> MCPRouteResult {
        do {
            switch uri {
            case "diffuse://workspaces":
                return try success(await queryService.listWorkspaces())
            case "diffuse://workspace/current/current-run",
                "diffuse://run/current/summary",
                "diffuse://run/current/files",
                "diffuse://run/current/symbols",
                "diffuse://run/current/findings":
                return try success(
                    await queryService.currentReviewContext(
                        options: AgentContextOptions(
                            detailLevel: .standard, includeFiles: true, includeSymbols: true)))
            case "diffuse://run/current/review-plan":
                return try success(await queryService.reviewPlan(runId: nil, focus: "all"))
            case "diffuse://run/current/profile":
                return try success(
                    await queryService.profileContext(
    ...
    func getPrompt(name: String) throws -> MCPPromptResult {
        let text: String
        switch name {
        case "diffuse.review_change":
            text =
                "Call diffuse.get_review_plan first, inspect high-risk files with diffuse.explain_file, then produce review findings grounded in file, line, symbol, or rule evidence."
        case "diffuse.understand_change":
            text =
                "Call diffuse.get_current_review_context and summarize the change using buckets, changed symbols, findings, skim targets, and profile evidence."
        case "diffuse.write_review_comment":
            text =
                "Use Diffuse findings and symbol/file explanations. Each review comment should cite a path and line range when available."
        default:
